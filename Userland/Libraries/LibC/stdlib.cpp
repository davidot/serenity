...
static T strtofloatingpoint(char const* str, char** endptr)
{
    ...
    if (auto starts_with_negative = view.starts_with("-0x"sv);
        ...
        // optionally followed by a '+' or '-' character, and then followed by one or more decimal digits
        double_parse_result = AK::parse_first_floating_point<T>(parse_ptr, parse_ptr + length);
    }
...
{
    return strtofloatingpoint<double>(str, endptr);
}
...
{
    return strtofloatingpoint<float>(str, endptr);
}
